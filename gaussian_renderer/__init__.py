...
def render(viewpoint_camera, pc : GaussianModel, pipe : PipelineParams, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, splat_args: ExtendedSettings = None, debugVis : DebugVisualization = DebugVisualization()):
    """
    Render the scene. 
...
def render_simple(viewpoint_camera, pc : GaussianModel, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
    ...
    Background tensor (bg_color) must be on GPU!
    """
    # all wrong by default (which is fine)
